(function () {
    ...
    ircClient.prototype.eventHandler = function (data) {
        if (data.method === 'PRIVMSG') {
            ...
            data.receiver = data.receiver.substring(0, (data.receiver.length));
            this.emit('JOIN', data);
        }
        else if (data.method === 'INVITE') {
            data.message = data.message[0].substring(1, (data.message[0].length));
            ...
            data.message[1] = data.message[1].substring(1, (data.message[1].length));
            this.emit('KICK', data);
        }
    ...
    function assembleResponse (response) {
        ...
        try {
            ...
            sender = (formatNick.substring(0,(formatNick.length)));   // nick => Done.
        } catch(e) {
            sender = undefined;
        }
